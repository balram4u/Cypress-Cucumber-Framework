...
class CommonUtilities {
  ...
  static validateMessage(message) {
    ...
    cy.xpath(CLOSE_ALERTMESSAGE).each(($el, index, $list) => {
      cy.wrap($el).click();
    });
  ...
  static readFileName(fileName) {
    ...
    cy.readFile(`${DOWNLOAD_PATH}${fileName}`, {
      timeout: 60000,
    });
  }
  ...
  static validateTableListParameters(columnName, columnData) {
    ...
    cy.log("Coulmn name ="+ columnName , "column data =" +columnData)
    cy.xpath(`//div[@class='MuiDataGrid-colCellTitle' and text()='${columnName}']/ancestor::div[@class='MuiDataGrid-main']/descendant::div[text()='${columnData}']`).then(($elements) => {
      const listOfElements = $elements.length;
      cy.log("List of Elements ="+ listOfElements);
      // eslint-disable-next-line no-plusplus
      for (let i = 1; i <= listOfElements; i++) {
        cy.xpath(`(//div[@class='MuiDataGrid-colCellTitle' and text()='${columnName}']/ancestor::div[@class='MuiDataGrid-main']/descendant::div[text()='${columnData}'])[${i}]`).should('include.text', columnData);
      }
    });
  }
